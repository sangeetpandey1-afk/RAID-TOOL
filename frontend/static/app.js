 ...
 *
 * No external dependencies. Talks to the same /api/* endpoints that the
 * Excel VBA front-end uses, so business logic lives only in the backend.
 * ================================================================== */
...
function statusPill(status) {
  ...
  else if (["noticed","approaching","provisional","section3","section5"].includes(s)) cls = "yellow";
...
async function refreshDashboard() {
  ...
  if (a.ok) {
    ...
    $("#alertsBox").textContent = "Could not load alerts: " + a.error;
  }
}
...
function renderDevices() {
  ...
  body.innerHTML = STATE.devices.map((d, i) => `
    <tr data-idx="${i}">
      ...
      <td class="num">${d.units !== "" ? Number(d.units).toFixed(2) : "—"}</td>
...
function collectDevicesFromUI() {
  // ensure row state matches inputs
...
async function liveCalc() {
  ...
  $("#calcResult").innerHTML = `
    <table class="data-table"><tbody>
      ...
      <tr><th>GRAND TOTAL</th><td class="num"><b>₹ ${fmtMoney(d.grand_total)}</b></td></tr>
    </tbody></table>
...
async function offenseCheckActive() {
  ...
  toast(`Offenses: ${h.total_offenses} · Repeat: ${d.is_repeat_offender} · Suggested ${d.suggested_multiplier}×`, "ok");
}

async function generateAllDocs() {
  const cid = STATE.currentCaseId;
  if (!cid) { toast("Save the case first", "error"); return; }
  const kinds = ["provisional_consumer", "provisional_office", "section3",
                 "section5", "thanedari", "envelope", "deposit_slip",
                 "compounding_order", "noc"];
  let ok = 0, fail = 0;
  for (const k of kinds) {
    const env = await API.post(`/api/cases/${cid}/document/${k}`);
    if (env.ok) ok++; else fail++;
  }
  toast(`Documents: ${ok} OK · ${fail} failed (saved under docs/${cid}/)`,
        fail ? "error" : "ok");
}

// ============================================================ CASES
async function refreshCases() {
  const env = await API.get("/api/cases/search?limit=200");
  ...
  const filtered = !filter ? list : list.filter(c =>
    [c.case_id, c.account_number, c.user_name, c.consumer_name, c.case_status,
     c.section, c.je_name].some(v => (v || "").toString().toLowerCase().includes(filter)));

  $("#casesBody").innerHTML = filtered.map(c => `
    <tr>
      <td><code>${escapeHtml(c.case_id)}</code></td>
      <td>${escapeHtml(c.account_number || "")}</td>
      <td>${escapeHtml(c.consumer_name || c.user_name || "")}</td>
      <td>${escapeHtml(c.section || "")}</td>
      <td>${escapeHtml(c.inspection_date || "")}</td>
      <td class="num">${fmtMoney(c.total_assessment)}</td>
      <td class="num">${fmtMoney(c.compounding_amount)}</td>
      <td>${statusPill(c.case_status)}</td>
      <td>
        <button class="btn small" data-load-case="${escapeHtml(c.case_id)}">Open</button>
      </td>
    </tr>
  `).join("");
}
...
$("#casesBody").addEventListener("click", async (e) => {
  const cid = e.target.dataset.loadCase;
  if (!cid) return;
  await loadCase(cid);
});
...
async function refreshNotices() {
  ...
  $("#noticesBody").innerHTML = list.map(n => `
    <tr>
      ...
      <td>${escapeHtml(n.notice_date || "")}</td>
      <td>${escapeHtml(n.due_date || "")}</td>
      <td>${statusPill(n.status)}</td>
      <td>${escapeHtml(n.dispatch_method || "")}</td>
    </tr>`).join("") || `<tr><td colspan="6" style="text-align:center;color:var(--muted)">No notices yet.</td></tr>`;
}
...
document.addEventListener("click", (e) => {
  ...
  switch (action) {
    ...
    case "generate-all-docs": generateAllDocs(); break;
