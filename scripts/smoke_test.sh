...
hr "1. health"
curl -s "$HOST/api/health" | python -m json.tool

hr "2. devices (sample)"
curl -s "$HOST/api/devices?category=Cooling" | python -m json.tool | head -40
...
curl -s -X POST "$HOST/api/compounding" -H 'Content-Type: application/json' \
  -d '{"load_w":2122,"category":"LMV-1","section":"135"}' | python -m json.tool
...
echo "$CASE_RESP" | python -m json.tool | head -40
CASE_ID=$(echo "$CASE_RESP" | python -c "import sys,json;print(json.load(sys.stdin)['data']['case']['case_id'])")
echo "Created case_id: $CASE_ID"

hr "6. fetch case"
curl -s "$HOST/api/cases/$CASE_ID" | python -m json.tool | head -40
...
curl -s -X POST "$HOST/api/cases/$CASE_ID/notices" -H 'Content-Type: application/json' \
  -d '{"notice_type":"provisional","notice_number":"PN-2025-001",
       "user":"test_user"}' | python -m json.tool
...
for k in provisional_consumer provisional_office section3 deposit_slip envelope compounding_order; do
  ...
  curl -s -X POST "$HOST/api/cases/$CASE_ID/document/$k" \
    -H 'Content-Type: application/json' -d '{}' | python -m json.tool | head -8
done

hr "11. dashboard summary"
curl -s "$HOST/api/dashboard/summary" | python -m json.tool
...
curl -s "$HOST/api/consumers/AC-TEST-001/offense-check" | python -m json.tool

hr "14. master files diagnostic"
curl -s "$HOST/api/master_files" | python -m json.tool

hr "15. import_all (no files yet, should report cleanly, NOT 500)"
curl -s -X POST "$HOST/api/import_all_master_data" | python -m json.tool | head -40
